...
class Admin_Blog_Model extends CI_Model {
	...
	function get_all_blogs() {
		return $this->db->order_by('blog_id','DESC')->get('blogs')->result_array();
	}
	...
	function change_blog_status() {
		...
		if ($this->db->where('blog_id',$this->input->post('id'))->update('blogs',$userdata)) {
			
			$log_data = $this->db->where('blog_id',$this->input->post('id'))->get('blogs')->row_array();
	...
	function get_blog_details() {
		return $this->db->where('blog_id',$this->input->post('blog_id'))->get('blogs')->row_array();
	}
	...
	function update_blog($blog_thumbnail_image) {
		...
		if ($this->db->where('blog_id',$this->input->post('blog_id'))->update('blogs',$update_data)) {

			$get_image = $this->db->select('blog_thumbnail_image')->where('blog_id',$this->input->post('blog_id'))->get('blogs')->row_array();
	...
	function delete_blog() {
		...
		if ($db_num_rows->num_rows() > 0) {
			...
			if($this->db->where('blog_id',$this->input->post('blog_id'))->delete('blogs')) {
