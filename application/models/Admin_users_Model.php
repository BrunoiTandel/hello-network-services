...
class Admin_users_Model extends CI_Model {
	...
	function get_analytics(){
		...
		$user_purchased_package = $this->db->query('Select * from user_purchased_package LEFT JOIN `users` ON `user_purchased_package`.`user_id` = `users`.`uid` where users.tag="'.$users['tag'].'"')->num_rows();
		$products = $this->db->get('products')->num_rows();
		$total = $this->db->query('Select sum(amount_paid) as amount_paid from user_purchased_package LEFT JOIN `users` ON `user_purchased_package`.`user_id` = `users`.`uid` where users.tag="'.$users['tag'].'"')->row_array();
		}
		return array(
			'user' => $user,
