...
(function (window) {
  function initProgressBar() {
    ...
    var createCheckPoints = function(stages, stageWidth, currentStageIndex) {
      var ul = createElement('ul', 'custom-progress-bar', { }, '');
...
(function (window) {
  function initProgressBar2() {
    var ProgressBar2 = {};
    ProgressBar2.singleStepAnimation = 1000; //default value
    // this delay is required as browser will need some time in rendering and then processing css animations.
    var renderingWaitDelay = 200;
    ...
    var createStatusBar = function(stages, stageWidth, currentStageIndex) {
      ...
      setTimeout(function() {
        currentStatus.style.width = (100 * currentStageIndex)/(stages.length - 1)+'%';
        currentStatus.style.transition = 'width '+(currentStageIndex * ProgressBar2.singleStepAnimation)+'ms linear';
      }, renderingWaitDelay);
    ...
    var createCheckPoints = function(stages, stageWidth, currentStageIndex) {
      ...
      for (var index = 0; index < stages.length; index++) {
        ...
        if(currentStageIndex >= index) {
          ...
          animationDelay+= ProgressBar2.singleStepAnimation;
        }
        ul.appendChild(li);
      }
      return ul;
    ...
    //exposing this function to user;
    ProgressBar2.init = function (stages, currentStage, container) {
      if(validateParameters(stages, currentStage, container)) {
        var wrapper = document.getElementsByClassName(container);
        if(wrapper.length > 0) {
          wrapper = wrapper[0];
        } else {
          wrapper = createElement('div', 'progressbar2-wrapper', { }, '');
          document.body.appendChild(wrapper);
        }
        createHTML(wrapper, stages, currentStage);
      }
  ...
  if (typeof ProgressBar2 === 'undefined') {
    window.ProgressBar2 = initProgressBar2();
  } else {
    console.log('Progress bar loaded');
  }
