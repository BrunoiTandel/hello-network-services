...
function not_mandatory_ip_address(variable_array) {
	var input_value = $(variable_array.input_id).val();
	if (input_value != '') {
		if (!ip_address_regex.test(input_value)) {
			$(variable_array.error_msg_div_id).html('<span class="text-danger error-msg-small">'+variable_array.not_an_ip_address_input_error_msg+'</span>');
			return 0;
		} else {
			$(variable_array.error_msg_div_id).html('');
			return 1;
...
function verify_admin_password(variable_array) {
	var input_value = $(variable_array.input_id).val();
	if (input_value != '') {
		$(variable_array.error_msg_div_id).html('');
		$.ajax({
	        ...
	        success : function(data) {
	        	if (data.count == 1) {
	        		if (variable_array.verify_for == 'payment-gateway-update') {
		            	payment_gateway_update();
		            } else if (variable_array.verify_for == 'update-farmer-pay-amount') {
...
function update_with_verifying_admin(variable_array) {
	$.ajax({
        type  : 'POST',
        url: base_url+variable_array.ajax_call_url,
        data : variable_array.ajax_pass_data,
        ...
        success : function(data) {
        	if (data.status == 1) {
        		$(variable_array.admin_error_msg_div_id).html('');
        		if (data.update_status.count == 1) {
        			if (data.update_status.status == 1) {
        				toastr.success(variable_array.success_message);
	                	$(variable_array.close_modal_id).modal('hide');
        			} else {
        				toastr.error(variable_array.error_message);
        			}
	            } else {
        			$(variable_array.admin_error_msg_div_id).html('<span class="text-danger error-msg-small">'+variable_array.incorrect_admin_password_error_message+'</span>');
	            }
        	} else {
	            toastr.error(variable_array.error_message);
        	}
