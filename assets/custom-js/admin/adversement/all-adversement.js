...
function get_all_blogs() {
	$.ajax({
        type  : 'POST',
        url   : base_url+'admin/get-all-adversements',
        data : {
	         	...
	         	if (data.all_blogs.length > 0) {
	         		...
	         		for (var i = 0; i < data.all_blogs.length; i++) {
				        ...
				        if (data.all_blogs[i].blog_status == '1') {
				       	...
				       	html += '<div class="col-md-4 mt-3" id="single-blog-main-div-'+data.all_blogs[i].adversement_id+'">';
                  		html += '<div class="latest-item">';
                  		html += '<div id="single-blog-image-div-'+data.all_blogs[i].adversement_id+'">';
                    	html += '<img src="'+img_base_url+'assets/uploads/blog-thumbnail/'+data.all_blogs[i].blog_thumbnail_image+'">';
                  		html += '</div>';
                    	html += '<h3 id="single-blog-blog-title-'+data.all_blogs[i].adversement_id+'">'+data.all_blogs[i].blog_title+'</h3>';
                    	html += '<div id="single-blog-content-'+data.all_blogs[i].adversement_id+'">'+data.all_blogs[i].blog_content.slice(0,max_blog_content_length)+'</div>';
                        ...
                        html += '<input type="checkbox" '+check+' onclick="change_blog_status('+data.all_blogs[i].adversement_id+','+data.all_blogs[i].blog_status+')" class="custom-control-input" id="change_blog_status_'+data.all_blogs[i].adversement_id+'">';
                        html += '<label class="custom-control-label" for="change_blog_status_'+data.all_blogs[i].adversement_id+'"></label>';
                        html += '</div></div>';
                      	html += '<div class="col-md-2 product-category-edit-delete">';
                        html += '<a class="product-category-delete-a" id="view_blog_'+data.all_blogs[i].adversement_id+'" onclick="view_blog_modal('+data.all_blogs[i].adversement_id+')"><i class="fa fa-eye edit-a ml-0"></i></a>';
                      	html += '</div>';
                      	html += '<div class="col-md-2 product-category-edit-delete">';
                        html += '<a class="product-category-delete-a" id="delete_blog_'+data.all_blogs[i].adversement_id+'" onclick="delete_blog_modal('+data.all_blogs[i].adversement_id+')"><i class="fa fa-trash edit-a text-danger ml-0"></i></a>';
                      	html += '</div>';
                    	html += '</div>';
                  		html += '</div>';
                		html += '</div>';
...
function change_blog_status(adversement_id,blog_status) {
	...
	var variable_array = {};
	variable_array['id'] = adversement_id;
	variable_array['actual_status'] = blog_status;
	variable_array['changed_status'] = changed_blog_status;
	variable_array['ajax_call_url'] = 'admin/change-adversement-status';
	variable_array['checkbox_id'] = '#change_blog_status_'+adversement_id;
	...
	variable_array['ajax_pass_data'] = {verify_admin_request : 1, id : adversement_id, changed_status : changed_blog_status};

	return change_status(variable_array);
}

function view_blog_modal(adversement_id) {
	$.ajax({
        type  : 'POST',
        url   : base_url+'admin/get-adversement-details',
        data : {
        	verify_admin_request : '1',
        	adversement_id : adversement_id
        },
        dataType : 'json',
        success : function(data) {
	        if (data.status == '1') {
                ...
                blog_image_html += '</div>';
		      	...
		      	$('#update-blog-btn').attr('onclick','update_blog('+adversement_id+')');
...
function update_blog(adversement_id) {
	...
	if (check_blog_title_var == 1 && check_external_link_var == 1 && check_post_type_var == 1 && blog_content != '') {
		...
		formdata.append('adversement_id',adversement_id);
		...
		$.ajax({
			type: "POST",
		  	url: base_url+"admin/update-adversement",
		  	data:formdata,
		  	dataType: 'json',
		    contentType: false,
		    processData: false,
		  		...
		  		if (data.status == 1) {
					...
					$('#blog-error-div').html('');
				  	if (data.update_blog.status == '1') {
				  		...
				  		$('#single-blog-image-div-'+adversement_id).html('<img src="'+img_base_url+'assets/uploads/blog-thumbnail/'+data.update_blog.blog_thumbnail_image+'">');
				  		$('#single-blog-blog-title-'+adversement_id).html(blog_title);
				  		$('#single-blog-content-'+adversement_id).html(blog_content.slice(0,max_blog_content_length));
						...
						$('#view-blog-details-modal').modal('hide');
				  		...
				  		$('#update-blog-btn').attr('onclick','update_blog('+newsadversement_id_id+')');
...
function delete_blog_modal(adversement_id) {
	$('#delete-blog-btn').attr('onclick','delete_blog('+adversement_id+')');
	$('#delete-blog-modal').modal('show');
}

function delete_blog(adversement_id) {
	var variable_array = {};
	variable_array['id'] = adversement_id;
	variable_array['ajax_call_url'] = 'admin/delete-adversement';
	...
	variable_array['delete_image_div_id'] = '#single-blog-main-div-'+adversement_id;
	variable_array['delete_btn_id'] = '#delete-blog-btn';
	variable_array['ajax_pass_data'] = {verify_admin_request : 1, adversement_id : adversement_id};
