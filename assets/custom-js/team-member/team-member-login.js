...
function adminLogin() {
	...
	if (email != '' && password != '' && email_regex.test(email) == true && password.length >= password_length) {
		...
		$.ajax({
			type: "POST",
		  	url: base_url+"team-member/verify-login-details",
		  	data: {email: email, password: password, check_team_member_request : 1},
		  	 dataType: "json",
		  	success: function(data) {
			  	if (data.status == '1') {
			  		window.location.href = base_url+"team-member/pages";
			  	} else {
